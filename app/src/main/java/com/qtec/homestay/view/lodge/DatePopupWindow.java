...
import com.qtec.homestay.databinding.PopupwindowHotelDateBinding;
import com.qtec.homestay.utils.CalendarUtil;
import com.qtec.homestay.view.lodge.AppUtil;
import com.qtec.homestay.view.lodge.DateInfo;
...
//                        mBinding.tvTime.setText("请选择离店时间");
  ...
  private void getoffsetDate(String startDate, String endDate, boolean status) {
    ...
    try {
      String newStartDate = startDate.substring(startDate.lastIndexOf("-") - 1, startDate.length());
      String newEndDate = endDate.substring(endDate.lastIndexOf("-") - 1, endDate.length());

      mBinding.tvInDate.setText(newStartDate);
      mBinding.tvOutDate.setText(newEndDate);

    } catch (Exception e) {
      e.printStackTrace();
    }

//    System.out.println("选择时间 startDate = " + startDate + " " + endDate);
    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
    Calendar c = Calendar.getInstance();
    ...
    protected void convert(BaseViewHolder helper, DayInfo item) {
      ...
      if (item.getStatus() == 0) {
        if (item.isSelect()) {
          ...
          (helper.getView(R.id.ll_bg)).setBackgroundColor(activity.getResources().getColor(R.color.green_00beaf));
        } else {
          //没选中状态
          helper.getView(R.id.tv_date).setVisibility(View.VISIBLE);
          helper.getView(R.id.tv_status).setVisibility(View.GONE);
...
//
//                try{
//                    String startDate = mList.get(startGroupPosition).getList().get(startchildPosition).getDate();
//                    startDate.substring(startDate.lastIndexOf("-")-1,startDate.length());
//                    helper.setText(R.id.tv_in_date,startDate);
//                }catch (Exception e){
//                    e.printStackTrace();
//                }
...
//                try{
//                    String endDate = mList.get(endGroupPosition).getList().get(endchildPosition).getDate();
//                    endDate.substring(endDate.lastIndexOf("-")-1,endDate.length());
//                    helper.setText(R.id.tv_out_date,endDate);
//                }catch (Exception e){
//                    e.printStackTrace();
//                }
